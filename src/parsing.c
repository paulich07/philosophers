...
{
	if (!is_number(argv[1]) || ft_strlen(argv[1]) > 12 || ft_atoi(argv[1]) <= 0)
		return (printf("Invalid number_of_philosophers\n"), 1);
	if (!is_number(argv[2]) || ft_strlen(argv[2]) > 12 || ft_atoi(argv[2]) <= 0)
		return (printf("Invalid time_to_die\n"), 1);
	if (!is_number(argv[3]) || ft_strlen(argv[3]) > 12 || ft_atoi(argv[3]) <= 0)
